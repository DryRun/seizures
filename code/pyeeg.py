...
def hfd(X, Kmax):
	...
	for k in xrange(1,Kmax+1):
		...
		for m in xrange(1,k+1):
			Lmk = 0
			for i in xrange(1,int(floor(1. * (N-m) / k))+1):
				Lmk += abs(X[m+k*i-1] - X[m+k*(i-1)-1])
			Lmk = Lmk * (N - 1.) / (floor(1. * (N - m) / k) * k * k)
			Lk.append(Lmk)
		L.append(log(mean(Lk)))
		x.append(log(1. / k))
	x_array = np.array(x)
	L_array = np.array(L)
	x_matrix = vstack([x_array, ones(len(x_array))]).T
	(p, r1, r2, s)=lstsq(x_matrix, L_array)
	return p[0]

def hfd2(X, Kmax):
	""" Attempt to speed up the HFD calculation
	"""
	N = len(X)
	x_matrix = np.empty((Kmax,2))
	L = np.empty((Kmax,1))
	for k in xrange(1, Kmax+1):
		Lk = 0.
		for m in xrange(0,k):
			# Sublist: X[0::k]
			Lk += np.sum(np.absolute(np.diff(X[m::k]))) * (N - 1.) / (floor(1. * (N - m) / k) * k * k)
		Lk = Lk / k
		L.itemset((k-1, 0), log(Lk))
		x_matrix.itemset((k-1, 0), log(1. / k))
		x_matrix.itemset((k-1, 1), 1.)
	(p, r1, r2, s)=lstsq(x_matrix, L)
	return p[0]
...
def dfa(X, Ave = None, L = None):
	...
	X:
		1-D Python list or numpy np.array
		a time series
	...
	if L is None:
		L = floor(len(X)*1/(2**np.array(range(4,int(log2(len(X)))-4))))
